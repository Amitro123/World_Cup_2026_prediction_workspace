...
4. **מפגשי עבר (head-to-head):** תוספת עליונות קטנה וחסומה מ-`data/h2h.csv` —
   נבחרת שניצחה בעבר את היריבה הספציפית מקבלת דחיפה. ידידותיים נספרים פחות
   (`H2H_FRIENDLY_W=0.4`), מפגשים ישנים דועכים (חצי-חיים 6 שנים), ומדגם קטן
   מתכווץ לעבר 0 (`H2H_SHRINK`). התרומה חסומה ל-±0.5 שער (`H2H_CAP`), כי דירוג
   פיפ"א כבר תופס את רוב החוזק. חל גם במגרש ניטרלי (היסטוריה נוסעת עם הצמד).
5. **בזמן אמת:** רק החלק הנותר של תוחלת השערים תקף (לפי הדקות שנותרו), מתווסף
...
משנה — הפרש השערים מתהפך אוטומטית לפי הנבחרת הביתית). `comp` = `friendly` או כל
ערך אחר (תחרותי). ניתן להרחיב ידנית או דרך Hermes ככל שמצטברים מפגשים.
